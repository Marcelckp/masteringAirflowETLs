...
def get_cities_and_states_from_csv(filename):
    ...
    with open(filename, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            cities_states.append((row['city'], row.get('state_id') or row.get('province_id') or ''))  # Append tuple (city, state/province)
...
def move_file_to_data_lake():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    source_path = './output/local_company_results.csv'
    destination_path = os.path.join(base_dir, '..', '..', '..', 'data_lake', 'raw', 'csv', 'local_company', 'local_company_results.csv')
...
centralized_filename = "local_company_results"

def get_local_company_data_with_playwright(selected_business_types, num_listings_to_capture, cities_states_original, headless, etl_run: bool = False):
    ...
    with sync_playwright() as p:
        ...
        for selected_business_type in selected_business_types:
                ...
                while listings_scraped < num_listings_to_capture:
                    ...
                    for listing in listings:
                        try:
                            ...
                            spinner_char = next(spinning_cursor())
                    ...
                    # If this is an ETL run, break after all locations are scraped
                    if listings_scraped >= num_listings_to_capture and etl_run is False:
                        break
            ...
            if business_list.business_list:
                business_list.save_to_csv(append=True, filename=centralized_filename)
                business_list.business_list.clear()
...
def collect_local_companies(etl_run: bool = False, num_listings_to_capture: int = 20, 
                            selected_business_types: list = None, headless: bool = True):
    if etl_run:
        print("ETL run detected. Using provided parameters.")
        cities_states_original = get_cities_and_states_from_csv('./local_company_lookup/southafrica_cities.csv')

        if selected_business_types is None:
            selected_business_types = business_types  # Use all business types if none specified

        get_local_company_data_with_playwright(
            selected_business_types=selected_business_types,
            num_listings_to_capture=num_listings_to_capture,
            cities_states_original=cities_states_original,
            headless=headless,
            etl_run=etl_run
        )
    else:
        ...
        get_local_company_data_with_playwright(selected_business_types, num_listings_to_capture, cities_states_original, headless, etl_run)

    move_file_to_data_lake()

if __name__ == "__main__":
    collect_local_companies(etl_run=False)